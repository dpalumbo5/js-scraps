...
//compare each word to every other word in the line
//start a variable at 0, if a word is valid add 1 to the variable
...
for (let i = 0; i < input.length; i++) {
  ...
  var answer = input.length - bad_passphrases
  console.log(answer);
...
for (let i = 0; i < input.length; i++) {
  ...
  function compare(row) {
    ...
    for (let j = 0; j < row.length; j++) {
     let something = (row[j].split(``).sort().join(`,`));
     sorted_words.push(something);
     sorted_words.sort();
    }
    
    for (let x = 0; x < sorted_words.length; x++){
      if(sorted_words[x] == sorted_words[x+1]) {
      bad_passphrases ++;
      break
      }
     }
